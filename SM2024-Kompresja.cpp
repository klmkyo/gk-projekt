...
int znajdzWSlowniku(const std::vector<slowo>& slownik, int rozmiarSlownika, slowo szukany) {
...
slowo polaczSlowo(slowo aktualneSlowo, uint8_t znak) {
    ...
    if (aktualneSlowo.dlugosc < 4096) {
        ...
        std::copy(std::begin(aktualneSlowo.element), std::begin(aktualneSlowo.element) + aktualneSlowo.dlugosc, std::begin(noweSlowo.element));
        noweSlowo.element[aktualneSlowo.dlugosc] = znak;
        return noweSlowo;
    } else {
        std::cout<<"UWAGA! przepelnienie rozmiaru znakow w pojedynczym slowie"<<std::endl;
        noweSlowo.kod = 0;
        noweSlowo.dlugosc = 0;
        noweSlowo.wSlowniku = false;
...
int dodajDoSlownika(std::vector<slowo>& slownik, int& rozmiarSlownika, slowo nowy, bool czyWyswietlac=true) {
    if (rozmiarSlownika<65536) {
        uint16_t nr = rozmiarSlownika;
        slownik[nr].kod = nr;
        slownik[nr].dlugosc = nowy.dlugosc;
        std::copy(std::begin(nowy.element), std::begin(nowy.element) + nowy.dlugosc, std::begin(slownik[nr].element));
...
std::pair<std::vector<int>, std::vector<slowo>> LZWKompresja(const int wejscie[], int dlugosc) {
    ...
    while (i < dlugosc) {
        ...
        if (kod < 0) {
            std::cout<<"NIE: ["<<kod<<"]"<<std::endl;
            dodajDoSlownika(slownik, rozmiarSlownika, slowoZnak);
            int kodAktualnegoSlowa = znajdzWSlowniku(slownik, rozmiarSlownika, aktualneSlowo);
            skompresowane.push_back(kodAktualnegoSlowa);
            std::cout<<"dodajemy do skompresowanych: "<<kodAktualnegoSlowa<<std::endl;
