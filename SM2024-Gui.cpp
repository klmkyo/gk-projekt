...
void Funkcja1() {
  int nieskompresowane[] = {0, 0, 0, 1, 1, 1, 1, 2, 0, 0, 3, 1,
...
void Funkcja7() {
  ...
  for (int yy = 0; yy < wysokosc / 2; yy++) {
    for (int xx = 0; xx < szerokosc / 2; xx++) {
      ...
      setPixel(xx + szerokosc / 2, yy + wysokosc / 2, ycbcr.cr, ycbcr.cr,
               ycbcr.cr);
    }
  }
...
void Funkcja9() {
  ...
  std::cout << "Loaded image dimensions: " << loadedImage[0].size() << "x"
            << loadedImage.size() << std::endl;
...
void paintCanvasFromOrigin(const Canvas &canvas, int tlx, int tly) {
  for (int yy = 0; yy < canvas.size(); yy++) {
    for (int xx = 0; xx < canvas[0].size(); xx++) {
      setPixel(xx + tlx, yy + tly, canvas[yy][xx].r, canvas[yy][xx].g,
               canvas[yy][xx].b);
    }
  }
}
...
void paintCanvasInCorner(const Canvas &canvas, int l, int b) {
  ...
  for (int yy = 0; yy < canvas.size(); yy++) {
    for (int xx = 0; xx < canvas[0].size(); xx++) {
      setPixel(xx + x, yy + y, canvas[yy][xx].r, canvas[yy][xx].g,
               canvas[yy][xx].b);
    }
  }
}
...
void setPixel(int x, int y, Uint8 R, Uint8 G, Uint8 B) {
  if ((x >= 0) && (x < szerokosc) && (y >= 0) && (y < wysokosc)) {
    ...
    Uint8 *p2 =
        (Uint8 *)screen->pixels + (y * 2 + 1) * screen->pitch + (x * 2) * bpp;
    Uint8 *p3 =
        (Uint8 *)screen->pixels + (y * 2) * screen->pitch + (x * 2 + 1) * bpp;
